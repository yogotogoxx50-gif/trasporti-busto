...
function findNextBuses(now, tipoGiornoCorrente) {
    ...
    orari.forEach(corsa => {
        // 1. Filtra per direzione, linea e fermata (NUOVA LOGICA CONFIG.JSON)
        const direzioneCorsa = corsa.Direzione ? corsa.Direzione.toLowerCase() : "";
        const lineaCorsa = corsa.Linea;
        const fermataCorsaLow = corsa.Fermata.toLowerCase();

        // Controlla se la direzione (andata/ritorno) esiste nel config
        if (!config.fermate_preferite || !config.fermate_preferite[direzioneCorsa]) return;

        // Controlla se la specifica linea ha fermate configurate per questa direzione
        const fermateConfigurate = config.fermate_preferite[direzioneCorsa][lineaCorsa];
        if (!fermateConfigurate) return;

        // Controlla se la fermata della corsa attuale è tra quelle configurate
        const matchFermata = fermateConfigurate.some(f => fermataCorsaLow.includes(f.toLowerCase()));
        if (!matchFermata) return;

        // 2. Controlla eccezioni linea (es. linea inattiva)
        if (config.eccezioni_per_linea && config.eccezioni_per_linea[lineaCorsa]) {
            const ecc = config.eccezioni_per_linea[lineaCorsa];
...
function renderResults(buses) {
    ...
    buses.slice(0, 15).forEach(bus => {
        ...
        card.innerHTML = `
            ...
            <p><strong>Fermata:</strong> ${bus.Fermata} ${bus.Comune && bus.Comune !== 'nan' ? '('+bus.Comune+')' : ''}</p>
            <p>${statoAttesa}</p>
        `;
